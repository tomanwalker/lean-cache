...
module.exports = function(size){
	...
	log('memory.init - start - size = %s', size);
	...
	this.removeByKey = function(key){
		...
		if(obj.next !== null){
			if( this.table[obj.next] && this.table[obj.next].prev ){
	...
	this.keys = function(){
		return Object.keys(_this.table);
	};
	
	this.tail = function(){
		return _this.table[_this.list.tail].value;
	};
	this.head = function(){
		return _this.table[_this.list.head].value;
	};
